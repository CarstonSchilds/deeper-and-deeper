...
var _calculated_random_patrol_time = 0.0
...
func _ready():
	rng.randomize()
	_calculated_random_patrol_time = rng.randf_range(-0.5, 0.5)
...
func enter(brain):
	...
	patrol_direction = brain.home + Vector2(-500.0, 00.0) # Somewhere far to the left
	threat = null
	...
	if brain.no_patrol:
		...
		going_left = true
		timer.wait_time = brain.normal_patrol_time + _calculated_random_patrol_time + 5
		timer.start()
